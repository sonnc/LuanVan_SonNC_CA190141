...
public class UdfController {
    ...
    public List<SysCodeSet> getAllCodeValue() {
        List<SysCodeSet> lstCodeSet = new ArrayList<>();
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            transaction = session.beginTransaction();
            Query query = session.createQuery("FROM SysCodeValue");
            lstCodeSet = query.list();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
    ...
    public List<SysCodeSet> getAllCodeSetActive() {
        List<SysCodeSet> lstCodeSet = new ArrayList<>();
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            transaction = session.beginTransaction();
            Query query = session.createQuery("FROM SysCodeSet where status='ACTIVE'");
            lstCodeSet = query.list();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
    ...
    public List<SysCodeSet> getAllCodeValueActive() {
        List<SysCodeSet> lstCodeSet = new ArrayList<>();
        try {
            session = HibernateUtil.getSessionFactory().openSession();
            transaction = session.beginTransaction();
            Query query = session.createQuery("FROM SysCodeValue where status='ACTIVE'");
            lstCodeSet = query.list();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
